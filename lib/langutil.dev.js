...
const langutil = {
    ...
    localize: function(keyword, paramArray=[]) {
        ...
        try {
            ...
            if (CONFIG_INITBYLANG) {
                localizedString = CONFIG_DICTIONARY[CONFIG_LANGUAGE][keyword];
            } else {
                localizedString = CONFIG_DICTIONARY[keyword][CONFIG_LANGUAGE];
            }
            localizedString = stringWithParams(localizedString, paramArray, CONFIG_SHOWLOGS);
            localizeSuccess = localizedString !== undefined;
        } catch (e) {
            if (CONFIG_SHOWLOGS) {
                if (error instanceof TypeError && CONFIG_DICTIONARY.length !== 0) {
                    console.warn('The localization for the keyword "' + keyword + '" has not been set for "' + CONFIG_LANGUAGE + '". ');
                } else {
                    console.error(error);
                }
            }
        }
        return localizeSuccess ? localizedString : ['_', keyword, '_'].join('');
    },
...
function inspectDictionary(newDictionary) {
    ...
    if (CONFIG_INITBYLANG) {
        allLanguages = Object.keys(newDictionary);
        for (i = 0; i < allLanguages.length; i++) {
            const languageSpecificLocalizations = newDictionary[allLanguages[i]];
            allKeywords.push(...Object.keys(languageSpecificLocalizations));
        }
        allKeywords = [...new Set(allKeywords)]; // Remove duplicate keys
    } else {
        var allKeywords = Object.keys(newDictionary);
        for (i = 0; i < allKeywords.length; i++) {
            const keywordSpecificLangauges = newDictionary[allKeywords[i]];
