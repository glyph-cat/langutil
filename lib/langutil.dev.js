...
const langutil = {
    ...
    init: (dictionary, language, autoDetect) => {
        ...
        if (Array.isArray(dictionary)) {
            ...
            B_setDictionary(formattedDictionary);
        } else {
            CONFIG_INITBYLANG = true;
            B_setDictionary(dictionary);
        }
        B_setLanguage(language, autoDetect);
    },

    setLanguage: (language, autoDetect) => { B_setLanguage(language, autoDetect); },

    createKey: (keyword, localizations) => ({ keyword: keyword, localizations: localizations }),

    localize: (keyword, paramArray=[]) => B_localize(keyword, paramArray),

    localizeWith: ({ keyword: keyword, paramArray: paramArray=[], casing: casing, transform: transform }) => {
        var localized = B_localize(keyword, paramArray);
        ...
        if (typeof localized === "string") {
            ...
            if (typeof casing === "undefined") {
                ...
                const casingFunc = {
                    ...
                    sentenceCase: (value) => B_capitalizeFirstLetter(value),
                    titleCase: (value) => {
                        value = value.split(/[\s]/g);
                        for (var i = 0; i < value.length; i++) {
                            value[i] = B_capitalizeFirstLetter(value[i]);
                        }
                        value = value.join(" ");
                        return value;
                    },
    ...
    getDefinedLanguages: () => B_getAllKwordAndDefLanguages(CONFIG_DICTIONARY).allLanguages,
...
function B_setLanguage(language, autoDetect) {
    language = autoDetect ? B_getAutoLanguage(language) : language;
...
function B_localize(keyword, paramArray=[]) {
    ...
    try {
        ...
        if (typeof localizedValue === "string") {
            localizedValue = B_stringWithParams(localizedValue, paramArray, CONFIG_SHOWLOGS);
 ...
 */
function B_getAutoLanguage(fallbackLanguage) {
    ...
    if (languageFound) {
        ...
        var allDefinedLang = B_getAllKwordAndDefLanguages(CONFIG_DICTIONARY).allLanguages;
        ...
        if (!dictHasDetectedLang) {
            ...
            if (!dictHasFuzzyLang) { langtoReturn = fallbackLanguage; }
            if (CONFIG_SHOWLOGS) {
 ...
 */
function B_getAllKwordAndDefLanguages(dictionary) {
 ...
 */
function B_capitalizeFirstLetter(value) {
 ...
 */
function B_getRandomString(pattern, length) {
    var newHash = "";
    do {
 ...
 */
function B_stringWithParams(stringWithFormat, paramArray=[], showLogs) {
    ...
    while (stringWithFormat.includes(escape_p_swapper)) {
        try {
            escape_p_swapper = B_getRandomString("0123456789abcdef", escape_p_Length++);
        } catch (e) {/**/}
    }
 ...
 */
function B_setDictionary(dictionary) {
    if (CONFIG_SHOWLOGS) {
...
function inspectDictionary(newDictionary) {
    ...
    var { allLanguages, allKeywords } = B_getAllKwordAndDefLanguages(newDictionary);
