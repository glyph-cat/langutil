...
const langutil = {
    ...
    logs: { focus, hide: () => { CONFIG_SHOWLOGS = false; }, show: () => { CONFIG_SHOWLOGS = true; } },
};
...
function localize(keyword, paramArray = []) {
    ...
    try {
        ...
        if (!localizeSuccess) { inspect.groupWarning(inspect.ellipsisQuote(keyword));; }
    } catch (e) {
        if (CONFIG_SHOWLOGS) {
