...
function appendDictionary(dict) {
...
const Casings = {
  lowerCase: CASING_TEMPLATES.lowerCase,
  upperCase: CASING_TEMPLATES.upperCase,
  localeLowerCase: CASING_TEMPLATES.localeLowerCase,
  localeUpperCase: CASING_TEMPLATES.localeUpperCase,
  sentenceCase: CASING_TEMPLATES.sentenceCase,
  titleCase: CASING_TEMPLATES.titleCase,
};
...
function applyCasing(value, casing, keyword) {
  if (typeof value === 'string') {
    ...
    if (typeof casing === 'string') {
      const applyTo = CASING_TEMPLATES[casing];
    ...
    if (!casingIsValid) {
      throw TypeError(`${formatInv({ expected: { casing }, toBeOneOf: Object.keys(CASING_TEMPLATES) })} (in '${keyword}')`);
    }
  } else {
...
module.exports = {
  // Standard
  init, setDictionary, appendDictionary, setLanguage, localize, langmap, getCurrentLanguage, getDefinedLanguages, AUTO_DETECT, logs, isAuto, Casings,
  // Deprecated
  createKey, getLanguage, localizeWith, snoozeInspectionUntil,
  // Internals - exploit at own risk
  _INTERNALS: {
    capitalizeFirstLetter, applyParam, applyCasing, applyTransform, getRandomHash, extractAB, convertToNewDict, formatInv, printWarning, flipDict, addListener, removeListener,
  }
};
