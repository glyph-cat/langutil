...
function setDictionary(dict) {
  ...
  config.initByLang = /[a-z]/.test(Object.keys(dictToUse)[0]);
...
function setLanguage(lang, detector) {
  ...
  const initialLang = config.lang, initialAuto = config.langIsAuto;
  ...
  if (config.lang !== initialLang || config.langIsAuto !== initialAuto) { updateHookedItems(); }
