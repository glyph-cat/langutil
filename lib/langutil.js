...
function appendDictionary(dict, key) {
  ...
  if (key && config.appendTracker.includes(`${key}`)) { return; }
  ...
  // Keep this in memory so that the same content is not appended every time
  // the scope of function is called (Refer to [Point A])
  config.appendTracker.push(`${key}`);
...
function setLanguage(lang, detector) {
  ...
  if (detector) {
    // This lets dev know that they have preferred auto-detection
    // since detector is already provided
    config.auto = true;
  ...
  if (langIsAssigned) {
    ...
    // Disabled because if auto-detect fail, state should still be under auto-detect
    // config.auto = true;
  } else {
